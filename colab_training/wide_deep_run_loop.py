...
def run_loop(name, train_input_fn, eval_input_fn, model_column_fn,
  ...
  average_loss_arr, label_mean_arr, loss_arr, prediction_mean_arr = [], [], [], []

  for n in range(flags_obj.train_epochs // flags_obj.epochs_between_evals):
    model.train(input_fn=train_input_fn, hooks=train_hooks)

    results = model.evaluate(input_fn=eval_input_fn)
    average_loss_arr.append(results['average_loss'])
    label_mean_arr.append(results['label/mean'])
    loss_arr.append(results['loss'])
    prediction_mean_arr.append(results['prediction/mean'])
  ...
  # Export the model
  export_dir_path = './tmp'
  export_model(model, flags_obj.model_type, export_dir_path,
             model_column_fn)
