...
def build_model_columns(dataset):
  ...
  user_gender = tf.feature_column.numeric_column(meals.GENDER_COLUMN)

  user_age = tf.feature_column.numeric_column(
      meals.AGE_COLUMN, shape=(meals.N_AGE,), dtype=tf.uint8)

  user_hobbies = tf.feature_column.numeric_column(
      meals.HOBBIES_COLUMN, shape=(meals.N_HOBBY,), dtype=tf.uint8)

  user_health = tf.feature_column.numeric_column(
      meals.HEALTH_COLUMN, shape=(meals.N_HEALTH,), dtype=tf.uint8)
